...
def create_death_date (death_txt):
	
	death_bits = death_txt.split(",")
	# death_bits[0] = death_bits[0].replace('.', " ")
	if len (death_bits) > 2:
			#put an exception here for Hagood Henry
		pass	
	for bit in death_bits:
		bit = bit.replace('.', " ")
		print bit
	# need code here to split the first of two-item lists 
	# try on space
	# then convert Jan / January -to 1,  Sep / Sept to 9 etc
	# also
	# what about removing st, nd, rd, th etc?


	# then we're going to have to 
	# create a date object as in preceding function
				...
				if find_substr in line_text:
					...
					# if "(d." found use that to split the string
 					...
 					if deceased:
 						...
 						'''
 						#this needs to be better - process the death date
 						if 'or 'in death_text:
 							death_certain = False

        				else: #do something here to split the death date
        					pass
        				'''
